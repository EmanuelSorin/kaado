...
export class KanjiComponent {
  ...
  nuevoKana(){
    ...
    if(this.kanjiN5){
      kanaArray = [ ...kanaArray, ...this.kanjiN5Array];
    }
    if(this.kanjiN4){
      kanaArray = [ ...kanaArray, ...this.kanjiN4Array];
    }
    if(this.kanjiN3){
      kanaArray = [ ...kanaArray, ...this.kanjiN3Array];
    }
    if(this.kanjiN2){
      kanaArray = [ ...kanaArray, ...this.kanjiN2Array];
    }
    if(this.kanjiN1){
      kanaArray = [ ...kanaArray, ...this.kanjiN1Array];
    }
  ...
  async adivinarTypeGame(event: Event){
    ...
    if(inputValue.toLowerCase() == this.adivinar.meaning.toLowerCase()){
  ...
  areSimilar(str1: string, str2: string): boolean {
    return this.stringComparisonService.compareStrings(str1, str2);
  }
  compareStrings(str1: string, str2: string, threshold: number = 0.8): boolean {
